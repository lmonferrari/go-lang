...
type CnabStatements struct {
	...
	Nfe                          string `json:"nfe"`
	SexoSacado                   string `json:"sexo_sacado"`
	NacionalidadeSacado          string `json:"nacionalidade_sacado"`
	EstadoCivilSacado            string `json:"estado_civil_sacado"`
	RgSacado                     string `json:"rg_sacado"`
	DataEmissaoRg                string `json:"data_emissao_rg"`
	ValorIofEntrada              string `json:"valor_iof_entrada"`
	TaxaContrato                 string `json:"taxa_contrato"`
...
func cnabToJson(filename string) ([]byte, error) {
	...
	for i, line := range lines {
		...
		if i == 0 {
			...
			cnabHeader.Branco2 = line[131:594]
			cnabHeader.NumSeqRegistro = line[594:600]
		} else if line[0:1] == "9" {
			cnabTrailer.IdRegistro = line[0:1]
			cnabTrailer.Branco = line[1:594]
			cnabTrailer.NumSeqRegistro = line[594:600]
			...
			if line[0:1] != "9" {
				...
				cnabStatements.Identificacao = line[181:182]
				...
				cnabStatements.Nfe = line[490:534]
				cnabStatements.SexoSacado = line[534:535]
				cnabStatements.NacionalidadeSacado = line[535:536]
				cnabStatements.EstadoCivilSacado = line[536:537]
				cnabStatements.RgSacado = line[537:548]
				cnabStatements.DataEmissaoRg = line[548:556]
				cnabStatements.ValorIofEntrada = line[556:567]
				cnabStatements.TaxaContrato = line[567:582]
				cnabStatements.Brancos4 = line[582:594]
				cnabStatements.NumSeqRegistro = line[594:600]
...
func main() {
	jsonBytes, err := cnabToJson("CB6200902.REM")
	if err != nil {
		log.Fatal(err)
	}

	err = os.WriteFile("CB6200902.json", jsonBytes, 0644)
	if err != nil {
		log.Fatal(err)
	}
}
